...
- Index `(run_id, occurred_at DESC)` where `run_id IS NOT NULL`.
...
- Index for unpublished claim scan: `(published_at, available_at, created_at)`.
...
6. Add consumer dedupe helper:
    - Insert `(consumer_name, event_id)` at the start of a handler transaction.
