...
The current implementation correctly requires `Idempotency-Key` for job submission, but stores submission idempotency
directly on `jobs`. The target architecture needs reusable idempotency for job submission, cancellation, retry,
schedules, admin quota changes, artifact finalization, and internal workflows.
...
- Partial unique index `(actor_type, actor_id, workflow, idempotency_key) WHERE tenant_id IS NULL`.
- Partial unique index `(tenant_id, actor_type, actor_id, workflow, idempotency_key) WHERE tenant_id IS NOT NULL`.
- Index `(status, locked_until)`.
- Check `status IN ('IN_PROGRESS', 'COMPLETED', 'FAILED_RETRYABLE', 'FAILED_FINAL')`.

Use a nullable `tenant_id` only while the app has no tenant/org model. Keep the column so the future migration is
additive. Do not use a single unique constraint with nullable `tenant_id`, because PostgreSQL treats `NULL` values as
distinct and would not dedupe tenantless requests.

`actor_id` is string-based to match `domain_events.actor_id` and support future service actors. User-scoped workflows
store the user UUID string.
...
- Idempotency records are operational dedupe state, not permanent audit history.
- A scheduled backend cleanup deletes all records older than the configured retention window.
- The default retention window is 24 hours.
- Stale `IN_PROGRESS` records are counted and logged before deletion because they indicate interrupted requests.
- Domain events and quota ledgers remain the long-lived audit sources.

## API Migration Steps

1. Keep existing `POST /jobs` behavior stable.
2. Add shared idempotency service behind `JobSubmissionService`.
3. Keep existing `jobs.idempotency_key` fields populated only while the schema still contains them; do not use them for
   correctness.
4. After `idempotency_records` is used in production flows, plan a later migration to remove submission-specific columns
   from `jobs`.

Implemented scope for this step:

- `job.submit` uses `idempotency_records` for correctness.
- Cancellation, retry, admin quota grant, and artifact finalization remain endpoint/domain contracts for later steps,
  because their run, quota grant, and artifact tables do not exist yet.
- Frontend job submission already generates one UUID key per form payload and forwards it through the Next route handler;
  this remains aligned with the shared backend contract.
...
Use for runner/control-plane finalization calls. Repeated finalization with the same manifest must return the same
result. Same key with different manifest hash must conflict.
