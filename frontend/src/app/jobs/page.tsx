...
import type { JobHistorySortDirection, JobsPageResponse } from './_components/types';
...
function buildJobsPath(page: number, size: number, sort: JobHistorySortDirection): string {
...
export default async function JobsPage({ searchParams }: { searchParams: JobsPageSearchParams }) {
  ...
  const jobsPath = buildJobsPath(page, size, sort);
  ...
  return (
    <div className="bg-background/50 min-h-[calc(100vh-3.5rem)] w-full py-8">
      <div className="container mx-auto max-w-6xl px-4 md:px-6">
        <JobsHistoryView jobsPage={jobsPage} />
      </div>
    </div>
  );
}
