...
(
    ...
    CONSTRAINT chk_idempotency_records_completed_response CHECK (
        status <> 'COMPLETED'
            OR (response_status IS NOT NULL AND response_body IS NOT NULL)
        )
);

CREATE UNIQUE INDEX uq_idempotency_records_scope_tenant_null
    ON idempotency_records (actor_type, actor_id, workflow, idempotency_key)
    WHERE tenant_id IS NULL;

CREATE UNIQUE INDEX uq_idempotency_records_scope_tenant_present
